document.addEventListener("DOMContentLoaded", () => {
  ...
  class TestTimer {
    ...
    updateTimerDisplay(seconds) {
      ...
      this.timerDisplay.textContent = `${minutes}:${secs < 10 ? "0" : ""}${secs}`;
    }
  }
  ...
  class TestController {
    constructor() {
      ...
      // Audios
      this.specialSound = new Audio("Audios/MULTILEVEL/Answer-sound.mp3");
      this.startingExamSound = new Audio("Audios/MULTILEVEL/Exam-start.mp3");
      this.sayYourName = new Audio("Audios/MULTILEVEL/Tell-Full-Name.mp3");
      this.thankYou = new Audio("Audios/MULTILEVEL/ThankYou.mp3");
      this.endAnswerTimeAudio = new Audio("Audios/MULTILEVEL/Time-Is-Up.mp3");
      this.endExamAudio = new Audio("Audios/MULTILEVEL/Exam-ended.mp3");
      this.yourExamStartsAudio = new Audio("Audios/MULTILEVEL/Exam-start-10s.mp3");

      this.introAudios = {
        "Part 1.1": new Audio("Audios/MULTILEVEL/Part1.1-intro.mp3"),
        "Part 1.2": new Audio("Audios/MULTILEVEL/Part1.2-intro.mp3"),
        "Part 2": new Audio("Audios/MULTILEVEL/Part2-intro.mp3"),
        "Part 3": new Audio("Audios/MULTILEVEL/Part3-intro.mp3"),
      };
    }
    ...
    async loadTest(testName) {
      const response = await fetch(`MULTILEVEL-Tests/${testName}/questions.json`);
    ...
    updateActivePartLabel(part) {
      document.querySelectorAll(".part-label").forEach((label) => label.classList.remove("active"));
      const activeLabel = document.getElementById(`part-${part.replace(/\./g, "-")}`);
      if (activeLabel) {
    ...
    displayImages(images) {
      ...
      this.imageDisplay.innerHTML = images
        .map((img) => `<img src="MULTILEVEL-Tests/${this.testName}/imgs/${img}" alt="Image" class="image-side-by-side" />`)
        .join("");
    }
    ...
    displayQuestions(questions) {
      ...
      const questionList = questions.map((q) => `<li style="text-align: left;">${Object.values(q)[0]}</li>`).join("");
    ...
    getPartInstruction(part) {
      const instructions = {
        "Part 1.1": "I will ask you a few questions about yourself. You have 30 seconds to answer.",
        "Part 1.2": "Look at these pictures. You will answer questions based on them. 45 seconds per answer.",
  ...
  // If testId is present in the URL, start the test automatically
  if (testId) {
    testController.startTest(testId);
  } else {
